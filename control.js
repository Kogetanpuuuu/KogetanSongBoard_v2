...
function save(){
  const data = {
    now: nowInput.value.trim(),
    requests: parseRequests(),
    requestOpen: requestOpen.checked,
    updatedAt: Date.now()
  };
  boardRef.set(data).then(() => showSaved("保存しました"));
}

boardRef.once("value").then(snap => {
  const data = snap.val() || {};
  nowInput.value = data.now || "";
  requestInput.value = Array.isArray(data.requests) ? data.requests.join("\n") : "";
